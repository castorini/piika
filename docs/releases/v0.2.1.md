...
Patch release focused on proving that `pi-search` now supports a real service-shaped backend path in addition to the Anserini BM25 adapter, and that `Piika` validates that contract correctly in the benchmark loop.
...
- proving through benchmark-harness regressions that `Piika` validates both successful and recoverable `pi-search` tool behavior over that path
